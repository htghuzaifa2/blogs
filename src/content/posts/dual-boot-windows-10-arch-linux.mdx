...
In this blog, Iâ€™ll walk you through how to safely install Arch Linux alongside Windows 10, covering BIOS/UEFI setup, partitioning, and GRUB bootloader installation.
...
2.  **Disable Fast Startup & Hibernation:**
    ...
    powercfg /h off
    ```
    Then go to **Control Panel â†’ Power Options â†’ Choose what power buttons do** and uncheck â€œTurn on fast startup.â€
3.  **Shrink Windows Partition:**
    -   Press **Win + X** â†’ **Disk Management**.
    -   Right-click your main drive (C:) and select **Shrink Volume**.
    -   Leave at least **40GB** free for Arch Linux. Youâ€™ll see unallocated space.
...
Run `lsblk` to find your drive (e.g., `/dev/sda`). Weâ€™ll use `cfdisk`:
```bash
cfdisk /dev/sda
```
...
3.  **(Optional) Swap:** 2â€“4GB, Type: `Linux swap`.
...
On reboot, youâ€™ll see the GRUB menu with options for "Arch Linux" and "Windows Boot Manager."

## Troubleshooting Tips

-   **GRUB doesnâ€™t show Windows:** Run `sudo os-prober` then `sudo grub-mkconfig -o /boot/grub/grub.cfg`.
-   **Boots directly into Windows:** Go into BIOS and set "Arch/GRUB" as the first boot option.
-   **Arch doesnâ€™t boot:** Use a live USB, `arch-chroot` into your system, and reinstall GRUB.

### Final Thoughts

Dual-booting Windows 10 and Arch Linux gives you the power to enjoy both ecosystems. If you follow this guide carefully, youâ€™ll have a smooth dual-boot setup with zero data loss.
