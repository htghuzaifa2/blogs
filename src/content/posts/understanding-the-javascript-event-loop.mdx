...
-   **The Web APIs (The Kitchen):** When you call `setTimeout`, `fetch`, or a DOM event, JavaScript hands that task to the browserâ€™s Web APIs. They handle the "Waiting" in the background so the Stack can stay clear.
...
-   **The Result:** The browser canâ€™t paint the UI, it canâ€™t handle clicks, and the user sees a "Page Unresponsive" error.
