---
title: How to Fix "Root Device Not Found" in Arch Linux (2025 Guide)
date: "2024-11-20"
author: Huzi
category: "Linux"
excerpt: "A step-by-step guide to troubleshoot and fix the 'root device not found' error in Arch Linux, covering mkinitcpio hooks, bootloader parameters, and hardware timing issues."
keywords: ["Arch Linux", "root device not found", "initramfs", "mkinitcpio", "GRUB", "systemd-boot", "Linux boot error", "troubleshooting", "kernel parameters"]
imageHint: "linux terminal error code"
---
...
**Verify the Root Partition's UUID:** From a live USB environment, use the `blkid` command to find the correct UUID of your root partition.

**Check Bootloader Entry:** Inspect your bootloader configuration file:
...
**Identify Needed Hooks:** From a live USB, chroot into your installed system and check the `/etc/mkinitcpio.conf` file. The `HOOKS` array must include hooks for your specific setup:
...
**Add Hooks and Rebuild:** Edit the `HOOKS` array in `/etc/mkinitcpio.conf`, placing necessary hooks (like `lvm2`) between `block` and `filesystems`. Save the file and regenerate the initramfs:
...
The key is to methodically check each part of the chain. If you can identify your root partition type (e.g., LVM, encrypted) or if you see any specific error messages in the emergency shell, you can get more targeted advice.
